...
const InputBox = (props) => {
    ...
    return (
        <Form.Field>
            <label>{label}</label>
            <input {...input} type={type} autoComplete={autoComplete} placeholder={placeholder} checked={checked} name={name} value={input.value} className={classNames} />
        </Form.Field>
...
let Demo = (props) => {
    ...
    return (
        <Form onSubmit={props.onSubmit}>
            <Field component={InputBox} type="text" autoComplete='firstName' label="First Name" name="patientmrn" val={demoInfo && demoInfo.demographics && demoInfo.demographics.patient && demoInfo.demographics.patient[0].mrn ? `${demoInfo.demographics.patient[0].mrn}` : ''} />
